...
###   franccore.org and franc.org and compares them.
...
WORKINGDIR="/tmp/franc_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://franccore.org"
HOST2="https://franc.org"
BASEDIR="/bin/"
VERSIONPREFIX="franc-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'franc-',
...
if [ $? -ne 0 ]; then
   echo "franc.org failed to provide signature file, but franccore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "franc.org and franccore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Franc Core binary release signing key installed?"
   fi
